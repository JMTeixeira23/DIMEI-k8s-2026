...
def provenance(docs, paths):
    ...
    for doc, path in zip(docs, paths):
        ...
        out.append(
            f"%   {cache_state(doc)}: {path}{asserted}"
        )
        out.append(
...
def rows_overhead(docs):
    ...
    header = ["Condition"]
    for d in docs:
        header += [f"{cache_state(d)}: client p50 (ms)",
                   f"{cache_state(d)}: in-webhook (ms)"]
    rows = []
    for cond in order:
        row = [cond]
        for d in docs:
...
def rows_size(docs):
    ...
    for d in docs:
        header += [f"{cache_state(d)}: in-webhook (ms)",
                   f"{cache_state(d)}: admitted"]
    rows = []
    for name in order:
        img = images.get(name, {})
...
def main():
    ...
    ap.add_argument(
        ...
        help="Column label for artefacts collected before the workflow recorded "
             ...
             "only where the state is documented elsewhere.")
    args = ap.parse_args()
        ...
        for d, label in zip(docs, args.cache_state):
            ...
            if env.get("image_verify_cache_enabled") not in (None, "", "unknown"):
                sys.exit("error: refusing to override a cache state the artefact "
                         "records. Drop --cache-state for these artefacts.")
            d["_supplied_state"] = label

    states = [cache_state(d) for d in docs]
    if len(set(states)) != len(states):
        print(f"warning: two artefacts report the same cache state ({states}). "
              "The comparison this table is for may not have been performed.",
              file=sys.stderr)
